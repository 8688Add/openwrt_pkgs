...
function get_outbound(cfg) {
	...
	if (type(cfg) === 'array') {
		...
		switch (cfg) {
		case 'block-out':
			/* block outbound was removed in sb 1.11+, use action 'reject' in route rules */
			return null;
		case 'direct-out':
			return cfg;
		default:
...
config.route = {
	rules: [
		...
		{
			inbound: filter(['mixed-in',
				match(proxy_mode, /redirect/) ? 'redirect-in' : null,
				match(proxy_mode, /tproxy/) ? 'tproxy-in' : null,
				match(proxy_mode, /tun/) ? 'tun-in' : null
			], (v) => v),
			action: 'sniff'
		}
	],
...
if (!isEmpty(main_node)) {
	...
	uci.foreach(uciconfig, uciroutingrule, (cfg) => {
		...
		/* block-out was removed in sb 1.11+, convert to action 'reject' */
		let rule_action = cfg.action;
		let rule_outbound = get_outbound(cfg.outbound);
